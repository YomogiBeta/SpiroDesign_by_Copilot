...
class FocusElement(Element):
    ...
    def addKeyEventListener(self, obj):
        element = obj.getElement()
        element.setAttribute('tabindex', '0')
        def listener(event): return self.onKeyEvent(event)
...
class HTML5Canvas(FocusElement):
    ...
    def fillRoundRect(self, x, y, width, heigh, radius):
        self.strokeRoundRect(x, y, width, heigh, radius)
        self._ctx.fill()

    def py_clear(self, x=0, y=0, width=None, height=None):
        # clear()
        if width is None:
            width = self.width
        if height is None:
    ...
    def strokeRoundRect(self, x, y, width, height, radius):
        self._ctx.beginPath()
        self._ctx.arc(x + radius, y + radius, radius, - math.pi, - 0.5 * math.pi, False)
        self._ctx.arc(x + width - radius, y + radius, radius, - 0.5 * math.pi, 0, False)
        self._ctx.arc(x + width - radius, y + height - radius, radius, 0, 0.5 * math.pi, False)
        self._ctx.arc(x + radius, y + height - radius, radius, 0.5 * math.pi, math.pi, False)
        self._ctx.closePath()
